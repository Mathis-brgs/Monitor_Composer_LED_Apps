...
### Phase 4 — Prérendu : producteur scène 3D → RT
But : une comp `kind:"prerender"` rend une vraie scène 3D (caméra + objets) dans sa RT, consommée comme un prérendu.
- [ ] `engine/Prerender3DScene.ts` : `Scene` Three.js + caméra (perspective/ortho, depth buffer) + meshes réels construits depuis les calques `shape`/objets de la comp (mapping `ShapeKind → Geometry`, matériaux depuis `Fill`). Rendu via `renderer.render(scene, camera)` → RT propre.
- [ ] Brancher la RT du prérendu sur le même `NestedTextureLayer` (Phase 3) : côté parent, un prérendu = une texture, indistinct d'une précomp.
- [ ] Animation interne : transforms/caméra/params pilotés par les tracks de la comp prérendu (réutilise l'évaluateur récursif).
- [ ] Réglages : `PrerenderScene` (caméra position/target/fov, background, résolution). UI minimale dans l'Inspector.
- [ ] Viewer : entrer dans un prérendu montre sa sortie 3D (vue caméra), scrubbée par le playhead local.
- Livrable : un objet 3D animé, rendu par caméra, appliqué comme source sur le mur LED.
